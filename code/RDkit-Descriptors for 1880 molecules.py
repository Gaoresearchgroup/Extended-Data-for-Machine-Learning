...
def calculate_descriptors_AL(smiles):
    ...
    sp2_carbons = sum(1 for atom in mol.GetAtoms() if
                      atom.GetAtomicNum() == 6 and atom.GetHybridization() == Chem.HybridizationType.SP2)
    ...
    conjugated_pi_electrons = sum(
        2 for bond in mol.GetBonds() if bond.GetBondType() in [Chem.BondType.DOUBLE, Chem.BondType.AROMATIC])
    conjugated_pi_electrons += sum(1 for atom in mol.GetAtoms() if
                                   atom.GetAtomicNum() == 6 and atom.GetHybridization() == Chem.HybridizationType.SP2)
...
if __name__ == '__main__':
    data = pd.read_excel('./Data/molecules2000.xlsx')
    print(data.shape)
    data['SMILES'] = data['SMILES'].apply(lambda x: Chem.MolToSmiles(Chem.MolFromSmiles(x)))
    descriptors_sol = data['SMILES'].apply(calculate_descriptors_AL)
    ...
    data_sol.to_excel('./Data/2000/data_AL.xlsx', index=False)
